...
fn test_alignment_with_nested_high_alignment() {
    ...
    assert_eq!(
        aligned_ptr as usize % 16,
        0,
        "Nested Aligned16 must be 16-byte aligned"
    );
}
...
fn test_alignment_with_query() {
    ...
    for (_id, component) in results {
        ...
        assert_eq!(
            ptr as usize % 16,
            0,
            "Queried component must be 16-byte aligned"
        );
    }
}
...
fn test_alignment_after_remove_and_readd() {
    ...
    for iteration in 0..10 {
        ...
        assert_eq!(
            ptr as usize % 16,
            0,
            "Must maintain alignment across add/remove cycles"
        );
...
fn test_large_alignment_struct() {
    ...
    let id = world.add_entity(Aligned32 { data: [1, 2, 3, 4] });
        ...
        let handle = thread::spawn(move || {
            for i in 0..100 {
                ...
                assert_eq!(
                    ptr as usize % 16,
                    0,
                    "Concurrent operations must maintain alignment"
                );
            }
        });
        handles.push(handle);
...
fn test_pointer_provenance_with_offset() {
    ...
    assert_eq!(
        ptr_b as usize % 16,
        0,
        "Nested field must maintain alignment"
    );
}
