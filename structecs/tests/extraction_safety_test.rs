...
fn test_extract_component_with_mutation() {
    ...
    let id = world.add_entity(MutableComponent { value: shared_value.clone() });
...
fn test_extract_concurrent_same_component() {
    ...
    for _ in 0..10 {
        ...
        let handle = thread::spawn(move || {
            for _ in 0..1000 {
                let comp = world_clone.extract_component::<Component1>(&id_clone).unwrap();
...
fn test_extract_with_large_component() {
    ...
    let id = world.add_entity(LargeComponent { data: large_data.clone() });
...
fn test_extract_with_high_alignment() {
    ...
    let world = World::new();
    let id = world.add_entity(HighAlignComponent { value: 0x1234567890ABCDEF });
...
fn test_extract_returns_error_for_missing_entity() {
    ...
    let id2 = world.add_entity(Component2 { data: "test".to_string() });
    assert!(world.extract_component::<Component1>(&id2).is_err());
}
